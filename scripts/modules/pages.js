  ...
  app.innerHTML = `
    <div class="auth-container">
      ...
      <form id="registerForm" class="auth-form">
        <input name="name" type="text" placeholder="Имя" required minlength="3" />
        <input name="login" type="text" placeholder="Логин" required minlength="3" />
...
export async function renderCommentsPage() {
      ...
      !auth
        ...
        : `<form id="commentForm" class="add-form">
             ...
             <textarea
               ...
               rows="4"
             ></textarea>
             <button type="submit" class="add-form-button">Написать</button>
           </form>
  ...
  try {
    const rawData = await getCommentsFromAPI();
    console.log('Сырые комментарии от API →', rawData);
    ...
    comments.splice(0, comments.length, ...normalized);
  } catch (err) {
